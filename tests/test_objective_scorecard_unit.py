...
def test_validate_metric_baseline_notes_hashes(tmp_path: Path) -> None:
    baseline = tmp_path / "docs/spec/metrics/a.json"
    _write(baseline, json.dumps({"x": 1}) + "\n")
    digest = hashlib.sha256(baseline.read_bytes()).hexdigest()
    notes = tmp_path / "docs/spec/metrics/baseline_update_notes.yaml"
    _write(
        notes,
        yaml.safe_dump(
            {
                ...
                "entries": [
                    {
                        "baseline": "docs/spec/metrics/a.json",
                        "sha256": digest,
                        "rationale": "update",
    ...
    violations = qm.validate_metric_baseline_notes(
        tmp_path,
        notes_path="docs/spec/metrics/baseline_update_notes.yaml",
        baseline_paths=["docs/spec/metrics/a.json"],
    )
    assert violations == []
