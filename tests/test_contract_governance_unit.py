...
def test_contract_governance_fails_when_regex_profile_linkage_is_missing(tmp_path):
    ...
    trace = {
        "links": [
            {
                "rule_id": "R8",
                "policy_ref": "docs/spec/contract/policy_v1.yaml#R8",
                "contract_refs": [f"docs/spec/contract/{x}" for x in _NORMATIVE_DOCS],
                "schema_refs": ["docs/spec/schema/schema_v1.md"],
                "conformance_case_ids": [],
                "unit_test_refs": ["tests/test_contract_governance_unit.py"],
...
def test_contract_governance_runtime_profile_can_relax_min_words(tmp_path):
    ...
    _write_text(
        tmp_path / "docs/spec/conformance/purpose_lint_v1.yaml",
        """version: 1
default:
  min_words: 8
...
""",
    ...
    _write_text(
        tmp_path / "spec_runner/bad.py",
        'PATTERN = "*.spec.md"\n',
    )
    errs = check_contract_governance(tmp_path)
    assert any("config literal duplicated outside settings" in e for e in errs)


def test_contract_governance_allows_runtime_python_using_settings_constants(tmp_path):
    _seed_governance_repo(tmp_path)
    _write_min_policy_trace(tmp_path, rule_id="R26")
    _write_text(
        tmp_path / "spec_runner/settings.py",
        """DEFAULT_CASE_FILE_PATTERN = "*.spec.md"
ENV_ASSERT_HEALTH = "SPEC_RUNNER_ASSERT_HEALTH"
