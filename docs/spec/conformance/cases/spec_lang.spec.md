...
type: text.file
requires:
  capabilities: [evaluate.spec_lang.ramda.v1]
  when_missing: skip
expect:
  portable: {status: skip, category: null}
  impl:
    python: {status: pass, category: null}
assert:
  - target: text
    must:
      - evaluate:
...
expect:
  ...
  impl:
    python:
      status: fail
      category: schema
      message_tokens: [over-application error]
assert:
  - target: text
    must:
      - evaluate:
          - ["call", ["call", ["var", "add"], 1], 2, 3]
```
