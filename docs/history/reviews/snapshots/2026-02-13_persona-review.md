...
Top unacceptable:
- no sandbox model for untrusted specs (`docs/spec/contract/04_harness.md`)
- policy stack may be too heavy for small teams (`docs/spec/contract/policy_v1.yaml`)
- env-driven behavior can still surprise (`spec_runner/harnesses/cli_run.py`)

Top salvageable:
- deterministic gate exists (`scripts/ci_gate.sh`)
- parity is executable, not rhetorical (`scripts/compare_conformance_parity.py`)
- contract/governance checks are concrete (`spec_runner/contract_governance.py`)
