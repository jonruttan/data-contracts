...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'required_rust_lane': {'workflow': '/.github/workflows/ci.yml', 'required_tokens': [{'ci-gate': None}, 'Run CI gate (required lane)', {'run': './scripts/ci_gate.sh'}], 'forbidden_tokens': ['Run CI gate (diagnostic lane)', 'continue-on-error: true\\n        run: ./scripts/ci_gate.sh']}, 'check': {'profile': 'governance.scan', 'config': {'check': 'runtime.required_rust_lane_blocking_status'}}, 'use': [{'ref': '/specs/05_libraries/policy/policy_core.spec.md', 'as': 'lib_policy_core_spec', 'symbols': ['policy.pass_when_no_violations']}]}"
services:
  actions:
