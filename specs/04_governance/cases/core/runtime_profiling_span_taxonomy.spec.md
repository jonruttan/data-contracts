...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'profiling_span_taxonomy': {'trace_path': 'specs/04_governance/cases/fixtures/run_trace_sample.json', 'required_spans': ['run.total', 'runner.dispatch', 'case.run', 'case.chain', 'case.harness', 'check.execute', 'subprocess.exec', 'subprocess.wait']}, 'check': {'profile': 'governance.scan', 'config': {'check': 'runtime.profiling_span_taxonomy'}}, 'use': [{'ref': '/specs/05_libraries/policy/policy_assertions.spec.md', 'as': 'lib_policy_core_spec', 'symbols': ['policy.assert.no_violations', 'policy.assert.summary_passed', 'policy.assert.summary_check_id', 'policy.assert.scan_pass']}]}"
contracts:
  clauses:
