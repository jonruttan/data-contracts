...
harness:
  ...
  config:
    ...
    - '{''root'': ''.'', ''local_prepush_broad_forbidden'': {''path'': ''/scripts/ci_gate.sh'', ''required_tokens'': [''skip broad governance (set SPEC_PREPUSH_REQUIRE_BROAD=1 to enable)'', ''SPEC_PREPUSH_REQUIRE_BROAD=1''], ''forbidden_tokens'': [''run_step governance "${SPEC_RUNNER_BIN}" governance'']}, ''check'': {''profile'': ''governance.scan'', ''config'': {''check'': ''runtime.local_prepush_broad_governance_forbidden''}}, ''use'': [{''ref'': ''/specs/05_libraries/policy/policy_assertions.spec.md'', ''as'': ''lib_policy_core_spec'', ''symbols'': [''policy.assert.no_violations'', ''policy.assert.summary_passed'', ''policy.assert.summary_check_id'', ''policy.assert.scan_pass'']}]}'
contracts:
  clauses:
