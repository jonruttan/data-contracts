...
| `services[]` | `adapters[].type` (required when `services` is present, or when `contracts.clauses[].bindings.rows[]` / `from: service` imports are used) | `/specs/schema/service_contract_catalog_v1.yaml` |
