...
- id: DCGOV-RUNTIME-STATUS-003
  ...
  purpose: Ensures status exchange ingestion is wired to release assets and 
    matrix artifacts.
  clauses:
    imports:
    - from: artifact
...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'status_ingestion': {'files': ['/scripts/runner_status_ingest.sh',
      '/specs/schema/runner_certification_registry_v2.yaml'], 'required_tokens': ['release_api_url',
...
services:
  ...
  - id: 
    ...
    type: 
      legacy.root_status_ingestion_files_scripts_runner_status_ingest_sh_specs_schema_runner_certification_registry_v2_yaml_required_tokens_release_api_url_report_asset_name_runner_status_matrix_json_runner_status_ingest_log_json_check_profile_governance_scan_config_check_runtime_compatibility_status_ingestion_configured
    io: io
    profile: default
    config: {}
```
