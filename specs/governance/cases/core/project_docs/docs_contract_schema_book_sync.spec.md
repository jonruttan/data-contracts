...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'doc_sync': {'files': ['docs/book/30_assertion_model.md', 'specs/contract/03_assertions.md', 'specs/schema/schema_v1.md'], 'tokens': ['MUST', 'MAY', 'MUST_NOT', 'contract.imports']}, 'check': {'profile': 'governance.scan', 'config': {'check': 'docs.contract_schema_book_sync'}}, 'use': [{'ref': '/specs/libraries/policy/policy_assertions.spec.md', 'as': 'lib_policy_core_spec', 'symbols': ['policy.assert.no_violations', 'policy.assert.summary_passed', 'policy.assert.summary_check_id', 'policy.assert.scan_pass']}]}"
services:
  entries:
