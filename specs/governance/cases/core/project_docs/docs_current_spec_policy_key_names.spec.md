...
- id: DCGOV-DOCS-CURRENT-KEYS-001
  ...
  purpose: Enforces policy expression naming consistency by allowing only 
    `evaluate` in `.spec.md` cases.
  clauses:
    imports:
    - from: artifact
      names:
...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'check': {'profile': 'governance.scan', 'config': {'check': 'docs.current_spec_policy_key_names'}},
...
services:
  ...
  - id: 
    ...
    type: 
      legacy.root_check_profile_governance_scan_config_check_docs_current_spec_policy_key_names_use_ref_specs_libraries_policy_policy_assertions_spec_md_as_lib_policy_core_spec_symbols_policy_assert_no_violations_policy_assert_summary_passed_policy_assert_summary_check_id_policy_assert_scan_pass
    io: io
    profile: default
    config: {}
    default: true
