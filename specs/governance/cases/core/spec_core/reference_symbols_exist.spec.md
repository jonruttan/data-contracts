...
- id: DCGOV-REF-SYMBOLS-001
  ...
  purpose: Ensures harness.spec_lang exports and library symbols resolve 
    deterministically.
  clauses:
    imports:
    - from: artifact
...
harness:
  ...
  config:
    ...
    - "{'root': '.', 'check': {'profile': 'governance.scan', 'config': {'check': 'reference.symbols_exist'}},
...
services:
  ...
  - id: 
    ...
    type: 
      legacy.root_check_profile_governance_scan_config_check_reference_symbols_exist_use_ref_specs_libraries_policy_policy_assertions_spec_md_as_lib_policy_core_spec_symbols_policy_assert_no_violations_policy_assert_summary_passed_policy_assert_summary_check_id_policy_assert_scan_pass
    io: io
    profile: default
    config: {}
    default: true
