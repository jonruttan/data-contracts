...
harness:
  ...
  config:
    ...
    - '{''root'': ''.'', ''when_fail'': {''path'': ''/dc-runner-python'', ''required_tokens'': [''fail_hook_ran'', ''if fail_hook_ran'', ''runtime.on_hook.fail_handler_failed'', ''"fail"'']}, ''check'': {''profile'': ''governance.scan'', ''config'': {''check'': ''runtime.when_fail_hook_required_behavior''}}}'
services:
  actions:
