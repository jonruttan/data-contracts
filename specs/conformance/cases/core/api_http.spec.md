...
- id: DCCONF-API-002
  ...
  purpose: Verifies api.http reports a schema violation when request url is 
    missing from portable fixture input.
  expect:
    portable:
      status: fail
      category: schema
...
- id: DCCONF-API-003
  ...
  purpose: Verifies extension capability gating can skip fixtures when a 
...
- id: DCCONF-API-004
  ...
  purpose: Verifies practical REST mutating verb support for POST requests in 
    deterministic mode.
  expect:
    portable:
      status: pass
...
- id: DCCONF-API-007
  ...
  purpose: Verifies practical REST verb support for DELETE in deterministic 
    mode.
  expect:
    portable:
      status: pass
...
- id: DCCONF-API-009
  ...
  purpose: Verifies practical REST verb support for OPTIONS in deterministic 
    mode.
  expect:
    portable:
      status: pass
...
- id: DCCONF-API-012
  ...
  purpose: Verifies requests scenario supports step templating and exposes 
    steps_json target.
  expect:
    portable:
      status: pass
...
- id: DCCONF-API-013
  ...
  purpose: Verifies oauth auth profile resolves env refs and produces oauth 
    context metadata without network access.
  expect:
    portable:
      status: pass
      category:
...
- id: DCCONF-API-016
  ...
  purpose: Verifies optional live oauth/network execution can be 
    capability-gated and skipped in portable lanes.
  requires:
    capabilities:
    - api.http
    - api.http.oauth.live
...
- id: DCCONF-API-017
  ...
  purpose: Maintains reference usage for domain.http CORS and scenario helper 
    symbol exports.
  requires:
    capabilities:
    - api.http
