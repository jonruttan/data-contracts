...
- `services[].operations[].imports` (effective via defaults+entry merge) MUST use canonical list[mapping] rows (`names`, optional `as`).
...
- Binding I/O surfaces (`contracts.clauses[].bindings.rows[].inputs/outputs`) MUST use canonical mapping rows only.
...
- `contracts.clauses[].asserts.imports` and `contracts.clauses[].asserts.checks[].imports` MUST use canonical rows (`{from, names, service?, as?}`).
